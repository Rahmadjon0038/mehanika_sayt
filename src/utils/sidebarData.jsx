
export const bolimlar = [
    {
        id: 1,
        name: "1-bob",
        bobname: "STATIKA.  ASOSIY  TUSHUNCHALAR.  AKSIОMALAR. BOG’LANISHLAR  VA ULARNING  REAKSIYA  KUCHLARI",
        children: [
            {
                id: 1,
                mavzu: "1.1 - § Kuch.  Kuchlar sistemasi. Teng ta’sir etuvchi kuch.",
                path: '/Bir_KuchKuchlarsistemasiTengsiretuvchikuch'
            },
            {

                id: 2,
                mavzu: "1.2 - § Asosiy aksiomalar.  ",
                path: '/AxiomsSection'
            },
            {
                id: 3,
                mavzu: "1.3 - § Uch kuch muvozanati haqidagi teoremaga doir masalalar",
                path: "/ForcesIntersection"
            },
            {
                id: 4,
                mavzu: "1.4 - § Kuchning o’qdagi va tekislikdagi prоeksiyasi",
                path: "/ProjectionAndProblems"
            },
            {
                id: 5,
                mavzu: "1.5 - § Nuqtaga nisbatan kuch momenti",
                path: "/ForceMoment"
            },
            {
                id: 6,
                mavzu: "1.6 - § Nuqtaga nisbatan kuch momentining vektori",
                path: "/MomentVector"
            },
            {
                id: 7,
                mavzu: "1.7 - § Varinion teoremasi",
                path: "/VarignonTheorem"
            },
            {
                id: 8,
                mavzu: "1.8 - § O’qqa nisbatan kuch mоmenti",
                path: "/ForceMomentOnAxis"
            },
            {
                id: 9,
                mavzu: "1.9 - § Juft kuch.  Juft momenti",
                path: "/CoupleMoment"
            },
            {
                id: 10,
                mavzu: "1.10 - § Bоg’lanish turlari va bоg’lanish reaksiyа kuchlari",
                path: "/ConnectionsAndReactions"
            },
         

        ]
    },

    {
        id: 2,
        name: "2-bob",
        bobname: " FAZODA IXTIYORIY JOYLASHGAN KUCHLARSISTEMASINI SODDA HOLGA KELTIRISH ",
        children: [
            {
                id: 1,
                mavzu: "2.1 - § Kesishuvchi kuchlar sistemasini qo’shish",
                path: '/KesishuvchiKuchlar'
            },
            {

                id: 2,
                mavzu: "2.2 - § Kesishuvchi kuchlar sistemasining muvоzanat tenglamalari",
                path: '/KesishuvchiKuchlarMuvozanati'
            },
            {

                id: 3,
                mavzu: "2.3 - § Kesishuvchi kuchlar sistemasining muvozanatiga doir masalalar sishuvchi kuchlar sistemasining muvоzanat tenglamalari",
                path: '/KesishuvchiKuchlarMuvozanatiMasala'
            },
            {

                id: 4,
                mavzu: " 2.4 – §. Bir tomonga qarab yo’nalgan o’zaro parallel ikkita kuchni qo’shish",
                path: '/BirTomongaYonalganParallelKuchlar'
            },
            {

                id: 5,
                mavzu: "2.5 - § Miqdorlari teng bo’lmagan, o’zaro parallel, qarama-qarshi tomonga yo’nalgan ikkita kuchni qo’shish",
                path: '/QaramaQarshiParallelKuchlar'
            },
            {

                id: 6,
                mavzu: "2.6 - § Parallel kuchlar markazi",
                path: '/ParallelKuchlarMarkazi'
            },
            {
                id: 7,
                mavzu: "2.7 - § Qattiq jismning og‘irlik markazi",
                path: '/OgirlikMarkazi'
            },
            {
                id: 8,
                mavzu: "2.8 - § Bir jinsli jismlarning og‘irlik markazini aniqlash",
                path: '/BirJinsliJismOgirlikMarkazi'
            },
            {
                id: 9,
                mavzu: "2.9 - § Bir jinsli jismlarning og‘irlik markazini aniqlash usullari ",
                path: '/OgirlikMarkaziYangi'
            },
            {
                id: 10,
                mavzu: "2.10 - § Oddiy shakilli, bir jinsli ba'zi jismlarning og‘irlik markazlarini aniqlash",
                path: '/YangiMalumot'
            },
            {
                id: 11,
                mavzu: "2.11 - § Kuchni berilgan markazga keltirish haqida lemma",
                path: '/OnBirKuchMarkazgaBerigan'
            },
            {
                id: 12,
                mavzu: "2.12 - § Statikaning asosiy teoremasi (Lui Puanso teoremasi)",
                path: '/StatikaningAsosiyTeoremasi'
            },

            {
                id: 13,
                mavzu: "2.13 - § Fazoda ixtiyoriy joylashgan kuchlar sistemasining muvozanati",
                path: '/FazodagiKuchlarMuvоzanati'
            },
        ]
    },

    {
        id: 3,
        name: "3-bob",
        bobname: " KUCHLAR  SISTEMASINING  MUVOZANAT  SHARTLARI. MASALALAR",
        children: [
            {
                id: 1,
                mavzu: "3.1 - § Tekislikda ixtiyoriy jоylashgan kuchlar sistemasining muvоzanati ",
                path: '/TekislikdaKuchlarMuvozanati'
            },
            {
                id: 2,
                mavzu: "3.2 - § yoyilgan kuchlarni hisobga olish",
                path: '/YoyilganKuchlarniHisobgaOlish'
            },
            {
                id: 3,
                mavzu: "3.3 - § Tekislikda joylashgan kuchlar sistemasining muvozanatiga doir masalalar",
                path: '/TekislikdaKuchlarSystemasiMuvozanati'
            },
            {
                id: 4,
                mavzu: "3.4 - § Statik aniq va statik aniqmas masalalar",
                path: '/StatikAniqVaStatikAniqmas'
            },
            {
                id: 5,
                mavzu: "3.5 - § Ishqalanishni hisоbga оlganda qattiq jismlar sistemasining muvоzanatini tekshirish. Sirpanishdagi ishqalanish",
                path: '/IshqalanishMuvozanat'
            },
            {
                id: 6,
                mavzu: "3.6 - § Ishqalanish konusi va ishqalanish burchagi",
                path: '/IshqalanishKonusi'
            },
            {
                id: 7,
                mavzu: "3.7 - § Ishqalanish koeffitsientini tajriba yo’li bilan aniqlash",
                path: '/IshqalanishKoeffitsientiTajriba'
            },
            {
                id: 8,
                mavzu: "3.8 – §. Dumalanib ishqalanish kuchi",
                path: '/DumalanibIshqalanishK38'
            },

        ]
    },


    // ===========================
    // 4-bo'lim
    {
        id: 4,
        name: "4-bob",
        bobname: " KINEMATIKA. NUQTA KINEMATIKASI",
        children: [
            {
                id: 1,
                mavzu: " 4.1 – §. Nuqta harakatining berilish usullari. Nuqta harakatining vektor usulda berilishi. Tezlik va tezlanish vektorlari.",
                path: '/Nuqtaharakati41'
            },
            {
                id: 2,
                mavzu: " 4.2 – §. Nuqta harakatining kооrdinatalar usulida berilishi. Harakat  koordinatalar usulida berilganda nuqtaning tezlik va tezlanishi.",
                path: '/NuqtaHarakatiKoordinatalarUsulida'
            },
            {
                id: 3,
                mavzu: " 4.3 – §.  Nuqta harakatining tabiiy usulda berilishi. Harakati tabiiy usulda berilgan nuqtaning tezlik va tezlanishi",
                path: '/NuqtaharakatiTabitU43'
            },
            {
                id: 4,
                mavzu: " 4.4 – §.Nuqta harakatini o'rganishga doir masalalar",
                path: '/NuqtaharakatiMasalalar44'
            },

        ]
    },
    // ====================================
    // 5-bolim
    {
        id: 5,
        name: "5-bob",
        bobname: "QATTIQ JISM KINEMATIKASI. QATTIQ JISMNING  SODDA HARAKATLARI",
        children: [
            {
                id: 1,
                mavzu: " 5.1 – §.  Qattiq jismning ilgarilanma harakati",
                path: '/IlgarilanmaHarakat51'
            },


            {
                id: 2,
                mavzu: " 5.2 – §. Qattiq jismning qo’zg’almas o’q atrofida aylanma harakati. Burchak tezlik va burchak tezlanish",
                path: '/QozgalmasOq52'
            },
            {
                id: 3,
                mavzu: " 5.3 – §.Burchak tezlik va burchak tezlanish vektorlari",
                path: '/BurchakTezlik53'
            },


            {
                id: 4,
                mavzu: " 5.4 – § Qo'zg'almas o'q atrofida aylanma harakatdagi jism ixtiyoriy nuqtasining tezligi",
                path: '/QozgalmaOq54'
            },
            {
                id: 5,
                mavzu: " 5.5 – § Qo’zg’almas o’q atrofidagi aylanma harakatdagi jism nuqtalarining tezlanishi",
                path: '/QozgalmasAylanmamain55'
            },
            {
                id: 6,
                mavzu: " 5.6 – § Qattiq jismning tekis parallel harakati",
                path: '/QattiqJismningHarakati56'
            },
            {
                id: 7,
                mavzu: " 5.8 – § Tekis parallel harakatdagi jismning harakat tenglamalari",
                path: '/TekisParalelHarakatDagi57'
            },
            {
                id: 8,
                mavzu: " 5.9 – § . Tekis shakl ixtiyoriy nuqtasi tezligini qutb nuqtadan foydalanib aniqlash",
                path: '/TekisShaklIxtiyoriy58'
            },
            {
                id: 9,
                mavzu: " 5.10 – § Tekis shakl nuqtalarining tezlanishi",
                path: '/TekisShaklTezlanishi59'
            },





        ]
    },
    // 6 bolim
    {
        id: 6,
        name: "6-bob",
        bobname: "NUQTANING  MURAKKAB  HARAKATI",
        children: [
            {
                id: 1,
                mavzu: " 6.1 – §.Asosiy tushunchalar. Absolyut harakat",
                path: '/AsosiyTushunchalar61'
            },
            {
                id: 2,
                mavzu: " 6.2 – §.Murakkab harakatdagi nuqtaning absolyut tеzligini aniqlash  ",
                path: '/MurakkabXarakattadagi62'
            },
            {
                id: 3,
                mavzu: " 6.3 – §.Murakkab harakatdagi nuqtaning absоlyut tеzlanishini aniqlash (Kоriоlis tеоrеmasi) ",
                path: '/Murakkabharakatdaginuqta63'
            },
        ]
    },

    // 7-bolim

    {
        id: 7,
        name: "7-bob",
        bobname: "MODDIY NUQTA DINAMIKASI",
        children: [
            {
                id: 1,
                mavzu: " 7.1 – §.Asosiy tushunchalar. Amalda uchraydigan ba’zi kuchlarning tasnifi.",
                path: '/Tushuncha71'
            },
            {
                id: 2,
                mavzu: " 7.2 – §. Dinamikaning asosiy qonunlari",
                path: '/DinamikaningAsosiyT72'
            },
            {
                id: 3,
                mavzu: " 7.3 – §.  Nuqta harakatini o'rganishga doir masalalar",
                path: '/MehanikOlchovBirliklari73'
            },
            {
                id: 4,
                mavzu: " 7.4 – § Moddiy nuqtaning harakat differensial tenglamalari",
                path: '/ModdiyNuqataDiferensialteng74'
            },
            {
                id: 5,
                mavzu: " 7.5 – §  Moddiy nuqta dinamikasining ikki asosiy masalasi",
                path: '/ModdinyNuqtaDInamikasi75'
            },
            {
                id: 6,
                mavzu: " 7.6 – § Moddiy nuqta dinamikasining birinchi asosiy masalasini yechishga doir masalalar",
                path: '/MOddiyNuqta76'
            },
            {
                id: 7,
                mavzu: " 7.7 – § Moddiy nuqta uchun Dalamber prinsipi (Kinetostatika usuli)",
                path: '/ModdiyNuqtaUchun77'
            },
            {
                id: 8,
                mavzu: " 7.8 – § Moddiy nuqta dinamikasining ikkinchi asosiy masalasini yechishga doir masalalar",
                path: '/ModdiyNuqta78'
            },
            {
                id: 9,
                mavzu: " 7.9 – §  Moddiy nuqta harakat miqdorining o’zgarishi haqidagi teorema",
                path: '/ModdiyNuqtaXarakat79'
            },
            {
                id: 10,
                mavzu: " 7.10 – § Moddiy nuqta kinetik energiyasi. Kuchning bajargan ishi. Quvvat",
                path: '/ModdiyNuqtaKinetika710'
            },
            {
                id: 11,
                mavzu: " 7.11 – § Qo’zg’almas o’q atrofida aylanma harakatdagi jismga qo’yilgan kuchning bajargan ishini hisoblash",
                path: '/QozgalmasOqAtrofi711'
            },
            {
                id: 12,
                mavzu: " 7.12 – § Mashinalarning mexanik foydali ish koeffitsiyenti",
                path: '/Mahinalar712'
            },
            {
                id: 13,
                mavzu: " 7.13 – § Mashinalarning mexanik foydali ish koeffitsiyenti",
                path: '/MddiyNuqtaTasir713'
            },
            {
                id: 13,
                mavzu: " 7.13 – §Moddiy nuqtaga ta’sir qiluvchi ba’zi bir kuchlarning bajargan ishini hisoblash. Masalalar",
                path: '/MddiyNuqtaTasir713'
            },
            {
                id: 14,
                mavzu: " 7.14 – § Moddiy nuqta kinetik energiyasining o’zgarishi haqidagi teorema. Masalalar",
                path: '/ModdiyNUqtaKinetik714'
            },
            {
                id: 15,
                mavzu: " 7.15 – § Nuqta kinetik energiyasining o’zgarishi haqidagi teoremani qo’llashga doir masalalar.",
                path: '/NuqTaKinetik715ma'
            },
        ]

        // =====================================
        // 8-BOLIM


    },

    // ===
    // 8-BOLIM
    {
        id: 8,
        name: "8-bob",
        bobname: "MODDIY NUQTALAR SISTEMASI DINAMIKASI  MASSALAR GEOMETRIYASI",
        children: [
            {
                id: 1,
                mavzu: " 8.1 – §. Mexanik sistema massasi. Mexanik sistema massalar markazi",
                path: '/Mehanika81'
            },
            {
                id: 2,
                mavzu: " 8.2 – §.Ichki va tashqi kuchlar. Ichki kuchlarning xossalari",
                path: '/IchkivaTashqi82'
            },
            {
                id: 3,
                mavzu: " 8.3 – §  Mexanik sistema harakat differensial tenglamalari",
                path: '/MexanikMasssi83'
            },
            {
                id: 4,
                mavzu: " 8.4 – § Massalar geometriyasi",
                path: '/MassalarGemetriyasi84'
            },
            {
                id: 5,
                mavzu: " 8.5 – § Markazga va o‘qqa nisbatan inersiya momentlari. Inersiya radiusi",
                path: '/MarkazgaVaOqqa85'
            },
            {
                id: 6,
                mavzu: " 8.6 – §Tekislikka nisbatan inersiya momenti. Koordinata o‘qlariga nisbatan inersiya momentlari. Bosh inersiya o‘qlari",
                path: '/TekislikksaNisBatan86'
            },
            {
                id: 7,
                mavzu: " 8.7 – § Jismning parallel o‘qlarga nisbatan inersiya momentlarini hisoblash (Gyuygens–Shteyner teoremasi)",
                path: '/JismNingParalel87'
            },

            {
                id: 8,
                mavzu: "  8.8– § Bir jinsli oddiy shaklli ba’zi jismlarning inersiya momentlarini hisoblash. Masalalar",
                path: '/BirjinsLIOddiyShakl88'
            },

        ]
    },

    //9-BOLIM
    {
        id: 9,
        name: "9-bob",
        bobname: "MEXANIK  SISTEMA  UCHUN  DINAMIKANING  UMUMIY   TEOREMALARI  VA  ULARNING  QATTIQ JISM HARAKATLARIGA  TATBIQI ",
        children: [
            {
                id: 1,
                mavzu: " 9.1 – §.Sistema massalar markazining harakati haqidagi teorema",
                path: '/SistemaMasalalar91'
            },

            {
                id: 2,
                mavzu: " 9.2 – §. Qattiq jismning ilgarilanma harakat differensial tenglamalari",
                path: '/Qattiqjismning92'
            },

            {
                id: 3,
                mavzu: " 9.3 – §   Mexanik sistema harakat miqdorining o‘zgarishi haqidagi teorema",
                path: '/MexankSisteme93'
            },
            {
                id: 4,
                mavzu: " 9.4 – §  Mexanik sistema harakat miqdori bosh vektorining o‘zgarishi haqidagi teorema",
                path: '/Sistemeharakat94'
            },

            {
                id: 5,
                mavzu: " 9.5 – § Mexanik sistemaning markazga va o‘qqa nisbatan kinetik momenti",
                path: '/Markazga95'
            },
            {
                id: 6,
                mavzu: " 9.6 – §  Mexanik sistema kinetik momentining o‘zgarishi haqidagi teorema. Masalalar",
                path: '/Kinetik96'
            },
            {
                id: 7,
                mavzu: " 9.7 – §  Qo‘zg‘almas o‘q atrofida aylanma harakatdagi qattiq jismning harakat differensial tenglamasi",
                path: '/QozgalmasOq97'
            },

            {
                id: 8,
                mavzu: "  9.8– § Qattiq jismning tekis parallel harakat differensial tenglamasi",
                path: '/QattiqJismning98'
            },
            {
                id: 9,
                mavzu: "  9.9– § Mexanik sistema kinetik energiyasi. Masalalar",
                path: '/MexanikSxema99'
            },
            {
                id: 10,
                mavzu: "  9.10 – § Mexanik sistema kinetik energiyasining o‘zgarishi haqidagi teorema",
                path: '/MexanikOzgarish910'
            },
            
            
        ]
    },
]
